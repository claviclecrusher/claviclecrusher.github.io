...
- 명단이 `config.js` 와 함께 브라우저로 내려갑니다. 저장소가 Public 이라
  누구나 파일을 열어 **전체 명단을 볼 수 있습니다.**
- 개발자도구를 쓰면 이름 없이도 내용을 볼 수 있습니다.
...
`config.js` 의 `guestGate.guests` 에 한 줄에 한 명씩 적습니다.
...
- 한 사람이 여러 이름으로 들어올 수 있으면 그냥 여러 줄 적으세요.
  (예: `'김영희',` 와 `'Kim Younghee',` 를 둘 다)
- 이름 뒤 쉼표 `,` 를 빠뜨리면 페이지가 통째로 안 뜹니다. 주의하세요.
- `/* ... */` 는 메모입니다. 화면에 안 나오니 자유롭게 쓰셔도 됩니다.
